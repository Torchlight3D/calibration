...
std::pair<double, double> Calibration::samplePeriod() const
{
    ...
    // chronological order.
    std::pair<double, double> period{kMaxDouble, kMinDouble};
...
// cv::Size <-> JSON
void to_json(json &j, const cv::Size &s)
{
    j = json::array({s.width, s.height});
}

void from_json(const json &j, cv::Size &s)
{
    s.width = j.at(0).get<int>();
    s.height = j.at(1).get<int>();
}

// AprilTag::Board::Options <-> JSON
void to_json(json &j, const tl::AprilTag::Board::Options &o)
{
    j = json{{"tagDim", {o.tagDim.width, o.tagDim.height}},
             {"tagSize", o.tagSize},
             {"tagSpacingRatio", o.tagSpacingRatio},
             {"startTagId", o.startTagId}};
}

void from_json(const json &j, tl::AprilTag::Board::Options &o)
{
    o.tagDim.width = j.at("tagDim").at(0).get<int>();
    o.tagDim.height = j.at("tagDim").at(1).get<int>();
    o.tagSize = j.at("tagSize").get<float>();
    o.tagSpacingRatio = j.at("tagSpacingRatio").get<float>();
    o.startTagId = j.at("startTagId").get<int>();
}
